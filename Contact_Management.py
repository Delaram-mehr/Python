...
class ContactManager:
    ...
    def validate_contact(self, name: str, number: str) -> bool: # Validate that the name and number are strings.
    ...
    def add(self, name: str, number: str) -> None: # Add a new contact, checking if the name already exists.

        if any(item['name'].lower() == name.lower() for item in self.contact_list):
            logging.warning(f"Contact with name '{name}' already exists.")
        else:
            if self.validate_contact(name, number):
    ...
    def edit(self, name: str, new_name: str = None, new_number: str = None) -> None: #Edit the contact's name or number based on the provided name.
    ...
    def search(self, name: str) -> list: # Search for contacts matching the given name
       
        return [item for item in self.contact_list if name.lower() in item["name"].lower()]
    

    def backup(self, path: str = "./contact_list.json") -> None: # Back up the contact list to the specified file path.
        
        try:
            with open(path, "w") as f:
                f.write(json.dumps(self.contact_list, indent=4))
            logging.info(f"Contacts backed up to {path}")
        except IOError as e:
            logging.error(f"Error saving contacts: {e}")


    def print(self) -> None: # Print all contacts in a readable format.
